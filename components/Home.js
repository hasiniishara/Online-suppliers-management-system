...
class Home extends React.Component {
  ...
  navPro3(){
    const { navigation } = this.props;
    this.props.navigation.navigate('ViewOrders',{emailArray:`${JSON.stringify(JSON.parse(navigation.getParam('emailArray')))}`});
  }

  navPro4(){
    const { navigation } = this.props;
    this.props.navigation.navigate('OrderHistory',{emailArray:`${JSON.stringify(JSON.parse(navigation.getParam('emailArray')))}`});
  }

  navPro(){
    const { navigation } = this.props;
    this.props.navigation.navigate('ViewProductDetails',{emailArray:`${JSON.stringify(JSON.parse(navigation.getParam('emailArray')))}`});
  }
